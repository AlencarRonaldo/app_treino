...
async function seedDatabase() {
  try {
    ...
    // Adicionar exercícios ao Treino A
    const supinoReto = exercises.find(e => e.name === "Supino Reto com Barra");
    const supinoInclinado = exercises.find(e => e.name === "Supino Inclinado com Halteres");
    const flexao = exercises.find(e => e.name === "Flexão de Braços");
    ...
    const barraFixa = exercises.find(e => e.name === "Barra Fixa");
    const remadaCurvada = exercises.find(e => e.name === "Remada Curvada com Barra");
    ...
    workouts.push(workoutB);

    // Treino HIIT
    const workoutHIIT = await prisma.workout.create({
      data: {
    ...
    const prancha = exercises.find(e => e.name === "Prancha Abdominal");

    if (burpee) {
      await prisma.workoutExercise.create({
        data: {
          workoutId: workoutHIIT.id,
